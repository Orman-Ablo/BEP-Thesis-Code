...
locdatafilename = 'localizations_BEP_Roman.mat';
filename = "C:\Users\Naam\Documents\BEP\data\Combined\localization_combined_initial_fit.mat";
%filename = "C:\Users\Naam\Documents\BEP\data\Combined\localization_combined_second_fit_final.mat";

load(filename,'localizations_drift_corrected','params')
...
maxlocxy = 50;%30; % max. for histogram
maxlocz = 100;%55; % max. for histogram

% %%%%%%%%% Filter central part of FOV
% xcenter = [0.333*params.pixelsize*params.imgSizeX 0.667*params.pixelsize*params.imgSizeX];
% ycenter = [0.333*params.pixelsize*params.imgSizeY 0.667*params.pixelsize*params.imgSizeY];
% %idx_center = find(localizations(:,2)>xcenter(1) & localizations(:,2)<xcenter(2) & localizations(:,3)>ycenter(1) & localizations(:,3)<ycenter(2));
%idx_center = find(localizations(:,2)>xcenter(1) & localizations(:,2)<xcenter(2) & localizations(:,3)>ycenter(1) & localizations(:,3)<ycenter(2) & localizations(:,5)<=8052);
%idx = find(localizations(:,5)<=8052);
%localizations = localizations(idx,:);
% %%%%%%%%%

% Our format
%localizations = localizations_drift_corrected;
Nmax = min(1e9,size(localizations,1)); %size(localizations,1);
% % setup array for multiframeconnect
numlocs = size(localizations(1:Nmax,:),1);
coords = localizations(1:Nmax,2:4);
framenumber = localizations(1:Nmax,5);
CRLBs = localizations(1:Nmax,6:8);
Nphs = localizations(1:Nmax,9);
bgs = localizations(1:Nmax,10);
runlengths_in = 1*ones(numlocs,1);
locindex = (1:numlocs)';

% SMAP format
% numlocs = size(saveloc.loc.xnm,1);
% coords = [saveloc.loc.xnm saveloc.loc.ynm saveloc.loc.znm];
% framenumber = saveloc.loc.frame;
% CRLBs = [saveloc.loc.locprecnm saveloc.loc.locprecnm saveloc.loc.locprecznm];
% Nphs = saveloc.loc.phot;
% bgs = saveloc.loc.bg;
% runlengths_in = 1*ones(numlocs,1);
% locindex = (1:numlocs)';
...
% Picasso format
% Nmax = min(1e8,size(localizations.x,1)); %size(localizations,1);
% pixelsize = 95.0;
% 
% localizations.z = double(zeros(size(localizations.x(1:Nmax))));
% coords = [localizations.x(1:Nmax) localizations.y(1:Nmax) localizations.z(1:Nmax)]*pixelsize;
% framenumber = localizations.frame(1:Nmax);
% CRLBs = [localizations.lpx(1:Nmax) localizations.lpy(1:Nmax) localizations.z(1:Nmax)]*pixelsize;
% Nphs = localizations.photons(1:Nmax);
% bgs = localizations.bg(1:Nmax);
% numlocs = Nmax;
% runlengths_in = 1*ones(numlocs,1);
% locindex = (1:numlocs)';
...
xlim([0 maxlocz])
subplot(2,3,4);
h = histogram(Nphs(locfilter),nbins);
Nphs_vv = h.BinEdges;
Nphcounts_vv = h.Values;
...
alldata = zeros(numlocs,20);
alldata(:,1:3) = coords;
alldata(:,4) = framenumber;
alldata(:,5:7) = CRLBs.^2;
alldata(:,8:10) = CRLBs;
alldata(:,11) = Nphs;
alldata(:,12) = bgs;
alldata(:,13) = runlengths_in;
alldata(:,14:16) = coords;
alldata(:,17:19) = coords.^2;
alldata(:,20) = locindex;
...
%%
output_path = 'C:\Users\Naam\Documents\BEP\BEP Code Roman\Linked_localizations\lidke_3d_linked_locs.mat';
%output_path = '/home/idroste/Desktop/TUDelft/Data/Lidke/data_analysis/DATA1/Data_w_Astigmatism/localization_SMAP/localizations_linked/Data_driftc_sml_linked.mat';
save(output_path,'numlocs_connect','coords_connect','index_connect','rstd_connect','CRLBs_connect','Nphs_connect','bg_connect','runlengths','rsum','rsqsum','bgmin','bgmax','Nphmin','Nphmin','maxdistxy','maxdistz','minrunlength','sigmafac','maxlocxy','maxlocz')
...
datadir = 'C:\Users\Naam\Documents\BEP\BEP Code Roman\Linked_localizations\';
filename = strcat(datadir,locdatafilename);
