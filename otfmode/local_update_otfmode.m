function [theta_local,thetastore_local,localizations,localizations_with_outliers,meritstore_local,alambda_local,mu_allspots,dmudtheta_allspots,Niters_local,outliers] = ...
         local_update_otfmode(theta_local,thetastore_local,meritstore_local,alambda_local,allspots,roixy,iiter_total,Niters_local,flip_z_net,framelist,ID,OTF_3d,params)
...
for jcfg = 1:Ncfg
    ...
    % Energy normalization of OTF (not used!)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % for iz = 1:size(OTF,3)
    %       energy = sum(abs(OTF(:,:,iz)).^2, 'all');
    %       OTF(:,:,iz) = OTF(:,:,iz) / sqrt(energy);
    % end
    % energy = sum(abs(OTF).^2, 'all');
    % OTF = OTF / sqrt(energy);
    % OTF = OTF/size(OTF,3);
    % disp(sum(OTF,'all'))% all have the same energy!
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [mu,dmudtheta] = poissonrate_otfmode(params,theta,OTF); % modified Poissonrate file, to work with OTF
    ...
    while ((iiter<=max_local_iterations) && ((iiter<=min_local_iterations) || (monitor>tollim)))
        ...
        [mutry,dmudthetatry] = poissonrate_otfmode(params,thetatry,OTF);
