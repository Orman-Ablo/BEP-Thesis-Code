%% Priority # 1: Determine accuracy of model PSF (mu) Made: How effectively is it made/does it work?
...
% 'local_update_otfmode' by a normal for loop, and comment the matrices
% storing the iterations in the while loop
...
output_path = 'C:\Users\Naam\Documents\BEP\data\3_localize\';
...
%% Note: The estimation of the PSF was performed over the distance set out by Zpatch (-503 to 503):
%% The spots to fit should probably fit  within this range too!
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if params.FlagOTF
    ...
    load(path_aberrations,'OTF_3d','PSFmodelmatrix','Xpatch','Ypatch','Zpatch');
...
params.check_test = true; % use part of the dataset only to check your results.
...
for file_i= nr_of_files
    ...
    if params.mult_run && params.FlagOTF
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %
        % let's keep it in for now: it's not used in OTFmode
        theta_global = loaded_data.theta_global;
    ...
    for batch = 1:Nbatch
        ...
        if params.cpp
            ...
            % do we need to flip the z-coordinates?
            flip_z_net = false; %true;%
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                ...
                local_update_otfmode(theta_init_batch,thetastore_local,theta_global,meritstore_local,alambda_local,spots_batch,roixy_batch,iiter_total,Niters_batch,flip_z_net,framelist(start+1:stop),ID(start+1:stop),OTF_3d,params); 
