...
% This file is modified from the original 'local_update'
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% precompute the patches for each spot,
roix = roixy(1,:);
roiy = roixy(2,:);
x_locs = theta_local(1,:) + roix * params.pixelsize;  % Fixed: use theta_local(1,:)
y_locs = theta_local(2,:) + roiy * params.pixelsize;  % Fixed: use theta_local(2,:)
...
for i = 1:Ncfg
    if x_locs(i) < Xpatch(1)
        x_patches(i) = 1;
    elseif x_locs(i) > Xpatch(end)
        x_patches(i) = Npatch;
    else
        patch_idx = find(x_locs(i) >= Xpatch(1:end-1) & x_locs(i) <= Xpatch(2:end), 1);
            x_patches(i) = patch_idx;
    end
end
...
for i = 1:Ncfg
    if y_locs(i) < Ypatch(1)
        y_patches(i) = 1;
    elseif y_locs(i) > Ypatch(end)
        y_patches(i) = Npatch;
    else
        % Find which patch this location belongs to
        patch_idx = find(y_locs(i) >= Ypatch(1:end-1) & y_locs(i) <= Ypatch(2:end), 1);
...
parfor jcfg = 1:Ncfg
    ...
    x_patch = x_patches(jcfg);
    y_patch = y_patches(jcfg);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% visualization of the check test
if params.check_test
    disp(size(pos_store))
    figure;
    hold on;
