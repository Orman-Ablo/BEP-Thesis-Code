function [PSF,PSFderivatives] = get_psfs_derivatives_otfmode(OTF,parameters,compders)
...
switch parameters.fitmodel
  case 'xyz'
    ...
    % disp(sum(abs(OTFtemp).^2,'all'))
    % L2 = sum(abs(OTFtemp).^2, 'all');
    % OTFtemp = OTFtemp / sqrt(L2);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if compders
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % OTF energy after positionphasemask
    % sprintf('OTF energy:')
    % E_OTF = sum(abs(OTF).^2,'all');
    % disp(E_OTF)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Psf energy (modified for energy conservation)
% PSF = cztfuncn2(OTF,allA,allB,allD,allOTFSize,allROISize,allM,allNotf,true);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This section actually brings the PSF to unity!
% disp(sum(PSF,'all'))
% Formation of unitary PSF.
total =sum(PSF,'all');
PSF = PSF/total;
% disp(sum(PSF,'all'))
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%         PSFderivatives(:,:,1,3) = transpose(cztfunc(IntermediateImage,Ax,Bx,Dx));
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % As as the photon count derivative is scaled by PSF, scale the
        % gradients by its energy too.
        PSFderivatives(:,:,1,1:3) = PSFderivatives(:,:,1,1:3)/total;
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
