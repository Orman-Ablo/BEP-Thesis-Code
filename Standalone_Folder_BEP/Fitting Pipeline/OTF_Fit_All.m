...
if params.FlagOTF
    ...
    intensity = mean(intensity, [2, 3]); % average the intensities for an axial fit
    params.mult_run = true;
    
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
spots_per_batch = 2e4; % sets the batch size
...
for file_i= 61:80
    ...
    if params.mult_run && params.FlagOTF % this statement picks the OTF fit
        ...
        Spots_fit = allspots; 
        roixy_fit = roixy; % We collect roixy_fit to describe all ROIs for the second fit, instead of for the first and second fit.
      
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %% Filtering for the localizations of initial fit
        % Use only the successfully filtered localizations of the initial
        % fit
        ...
        locs = loaded_data.localizations_with_outliers;
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % % This code filters out all localizations outside the defined
        % regions of the PSF models.
        % %
        % z_fit = [min(Zpatch) max(Zpatch)];
        % x_fit = [min(Xpatch) max(Xpatch)];
        % y_fit = [min(Ypatch) max(Ypatch)];
        % 
        % sep = find((locs(:,4)>= z_fit(1)) & (locs(:,4)<= z_fit(2))&(locs(:,3)>= y_fit(1)) & (locs(:,3)<= y_fit(2))&(locs(:,2)>= x_fit(1)) & (locs(:,2)<= x_fit(2)));
        % ID = ID(sep);
        ...
        params.pos = linspace(-params.Mpsfz/2,params.Mpsfz/2,params.Mpsfz);% note: for positions beyond the limits, the scale is extrapolated and can go negative!
        params.zstage = zemit; % not accessed
        params.compders =1;
        ...
        params.zspread = [-1000 1000]; % Since we have insufficient spots beyond the limits, what happens if we extrapolate?
    ...
    if params.use_fitted_aberrations
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % OTFmode doesn't use theta_global or any aberration
    ...
    for batch = 1:Nbatch
        ...
        if params.cpp
            ...
            flip_z_net = false;% not used for OTF fit
    ...
    filename = all_input_files(file_i).name; 
    path_output_data_full = strcat(output_path,'localization_OTF_',num2str(params.zspread(2)),filename);
