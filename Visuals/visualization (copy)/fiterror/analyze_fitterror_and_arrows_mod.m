...
% path_beadaber = "C:\Users\Naam\Documents\BEP\BEP Code Roman\data\final\localization_OTF_localization_segmentation_roi17_thr15p5_Data0001.mat";
...
path_beadaber = "C:\Users\Naam\Documents\BEP\BEP Code Roman\data\Localization_fullspread_alllocs\localization_OTF_1000localization_segmentation_roi17_thr15p5_Data0001.mat";
path_smlmaber = "C:\Users\Naam\Documents\BEP\BEP Code Roman\input data\transfer_3055388_files_4e4cf1e2\localization_segmentation_roi17_thr15p5_Data0001.mat";
...
%% Note: you need to align the spots with their respective positions and locations
...
% select only spots present in both fits
[C,index_1,index_2] = intersect(localizations_1(:,1),localizations_2(:,1));
mu_beadaber = mu_beadaber(:,:,:,index_1);
mu_smlmaber = mu_smlmaber(:,:,:,index_2);
roixy = roixy(:,index_2);
allspots = allspots(:,:,:,index_2);
z_positions = data_second_fit.localizations(index_2,4)';

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Remove all spot regions with negative values
reshaped = reshape(mu_smlmaber, [], size(mu_smlmaber,4)); % reshape spots to size
spot_mask = any(reshaped < 0, 1);% find all negative positions 
neg_spots = find(~spot_mask); % return spot index

mu_beadaber = mu_beadaber(:,:,:,neg_spots);
mu_smlmaber = mu_smlmaber(:,:,:,neg_spots);
roixy = roixy(:,neg_spots);
allspots = allspots(:,:,:,neg_spots);
z_positions = data_second_fit.localizations(neg_spots,4)';
...
%% Modify the zrange to observe different fitting performance!

% %% For the second fit, the PSF is only mapped from -500nm to 500 nm.
%zrange = [-503.5 503.5];
% %zrange = [-503.5 -200];
% zrange = [0 503.5];
% %%
...
xlabel('x (\mum)')
ylabel('y (\mum)')
title('\chi^{2}: OTF vs Initial Fit')
axis equal
colormap(redblue)
...
% cmap(1:n/2,:) = [linspace(green(1), white(1), n/2)', linspace(green(3), white(3), n/2)'];
% cmap(n/2+1:n,:) = [linspace(white(1), red(1), n/2)', linspace(white(3), red(3), n/2)'];
% 
% 
% % Apply the colormap
% colormap(cmap);
